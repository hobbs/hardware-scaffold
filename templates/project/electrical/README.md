...
4. ERC, local PDF export, and manual rating/pin-sharing review.
...
`kicad-toolcheck`, `kicad-erc`, and `kicad-pdf` targets from
`templates/project/Makefile` into the project's own build command. Then run
`make kicad-erc` and `make kicad-pdf`, or override
`KICAD_SCH=path/to/file.kicad_sch` and `KICAD_PDF=path/to/output.pdf`.
